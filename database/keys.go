...
// execExpireAt 设置一个键的过期时间（以Unix时间戳为单位）
...
// execExpireTime 返回给定键的绝对Unix过期时间戳（以秒为单位）
...
// execPersist 移除一个键的过期时间，使其变为持久化键
